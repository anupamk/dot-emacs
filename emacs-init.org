...
(use-package project
  ...
  (setq anupamk-project-root-markers '("Cargo.toml"
                                       "WORKSPACE"
  ...
  ;;
  ;; presence of *one* of marker files (returned from
  ;; `anupamk-make-project-root-markers') marks a directory as
  ;; project-root.
  ;;
  ;; search recursively, starting from `dirname' looking for these
  ;; marker files, and return project-rootdir or nil (when no marker
  ;; file exists)
  (defun anupamk-do-locate-project-root (dirname all-marker-list)
    ...
    (when all-marker-list
      (let ((project-rootdir (locate-dominating-file dirname (car
        ...
        (if project-rootdir
            ;; -----------------------------------------------------------------
            ;; found a live one. `transient' because, this project was
            ;; detected via our function.
            (cons 'transient project-rootdir)
  ...
  ;; ---------------------------------------------------------------------------
  ;; get the root directory only, instead of the alist that is
  ;; returned
  (defun anupamk-locate-project-root-dirname (&optional dirname)
    "return the root directory name a project"
    (unless dirname (setq dirname default-directory))
    (alist-get 'vc (list (anupamk-locate-project-root dirname))))
