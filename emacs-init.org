...
(use-package rg
  ...
  (rg-define-search anupamk:ripgrep-search/rg-vc-or-dir
    ...
    ;; current directory
    :dir (let ((project-root-dir (anupamk:project-utils/locate-project-root)))
           (if project-root-dir
               project-root-dir
             default-directory))
...
Similar to =ripgrep= but seems to be fasta !
...
(defadvice deadgrep-search-items (after anupamk:utils/recenter activate)
...
(use-package emacs
  ...
  ;; returned
  (defun anupamk:project-utils/get-project-root-dirname (&optional dirname)
    "return the root directory name a project"
    (unless dirname (setq dirname default-directory))
    (alist-get 'vc (list (anupamk:project-utils/locate-project-root dirname))))
