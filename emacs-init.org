...
(use-package project
  ...
  ;; ---------------------------------------------------------------------------
  ;; list of marker-filenames that define project-root
  (setq anupamk-project-root-markers '("Cargo.toml" ; rust-root
  ...
  ;; ---------------------------------------------------------------------------
  ;; return project-root list that is expected of `project-find-functions'
  ;;
  ;; starting from 'dirname' (and recursively searching up the
  ;; directory tree), mark the first directory as project-root if it
  ;; contains project-root-marker as defined in the
  ;; `anupamk-project-root-markers' list.
  ;;
  ;; when a suitable root-directory is located, the list
  ;;    ('vc <project-vc-backend> <project-root-directory>)
  ;; is returned.
  ;;
  ;; so, if you are just looking for the 'project-root-directory-name'
  ;; you need to get the 2nd element from the returned list.
  (defun anupamk-do-locate-project-root (dirname all-marker-list)
    "find project-root-directory by recursively searching starting from
    DIRNAME for project-root markers defined in ALL-MARKER-LISTS"

    (when all-marker-list
      (let ((project-rootdir (locate-dominating-file dirname (car
                                                              all-marker-list)))
            (backend (ignore-errors (vc-responsible-backend project-rootdir))))

        ;; ---------------------------------------------------------------------
        ;; found a live one.
        (if project-rootdir
            (list 'vc backend project-rootdir)

          ;; -------------------------------------------------------------------
          ;; keep looking
          (anupamk-do-locate-project-root dirname (cdr all-marker-list)))
        )))

  ;; ---------------------------------------------------------------------------
  ;; just a wrapper over the real thaang.
  (defun anupamk-locate-project-root (dirname)
    (anupamk-do-locate-project-root dirname anupamk-project-root-markers))

  ;; ---------------------------------------------------------------------------
  ;; return the project-root-directory name.
  (defun anupamk-locate-project-root-dirname (&optional dirname)
    "return a project's root directory name"
    (interactive)
    (nth 2 (anupamk-locate-project-root
            ;; -----------------------------------------------------------------
            ;; 'dirname' when specified, `default-directory' otherwise.
            (or dirname default-directory))))

  ;; ---------------------------------------------------------------------------
  ;; why both 'anupamk-locate-project-root' and
  ;; 'anupamk-locate-project-root-dirname' are required ?
  ;;
  ;; well, 'project-find-functions' expects the whole tamale as
  ;; returned from 'anupamk-locate-project-root' while, for `ripgrep'
  ;; (when searching from the 'top-of-project-root-dir' we just care
  ;; about the project-root-dirname)
  ;; ---------------------------------------------------------------------------

  ;; ---------------------------------------------------------------------------
  ;; cannot use `hook' because `project-find-functions' does not end
  ;; in `-hook'. also, must be defined in the `:config' section (of use-package)
  ;; because otherwise, 'project-find-functions' is not yet initialized.
  (add-hook 'project-find-functions #'anupamk-locate-project-root)

  :bind
  (("C-c p n" . project-switch-project)
...
Make the modeline less busy, by purging bunch of major/minor-modes
...
(use-package emacs
  :config
  ;; -----------------------------------------------------------------------------
  ;; disable a bunch of minor/major modes, otherwise the modeline look
  ;; too busy
  (defvar anupamk-hidden-minor-modes '(auto-fill-function
                                       which-key-mode
                                       whitespace-mode
  ...
  (defun anupamk-purge-minor-modes ()
    (dolist (ith-mm anupamk-hidden-minor-modes nil)
      (let ((hidden-mm (cdr (assoc ith-mm minor-mode-alist))))
        (when hidden-mm
          (setcar hidden-mm "")))))

  :hook
  (after-change-major-mode . anupamk-purge-minor-modes)
...
(defvar personal-attribute-table
  #s(hash-table
     ...
     data (
           ...
           "cc-mode:lsp-server-with-args"     ("ccls")
           "cc-mode:clang-format-executable"  "clang-format"
           ))
  )
...
(defvar tarana-attributes-table
  #s(hash-table
     ...
     data (
           ...
           "cc-mode:fill-column"              80
           "cc-mode:comment-column"           80
           "cc-mode:lsp-server-with-args"     ("/twbuild/anupam.kapoor/local/usr/bin/ccls")
           "cc-mode:clang-format-executable"  "/twbuild/anupam.kapoor/local/usr/bin/clang-format"
           ))
  )
...
(add-to-list 'fontaine-presets
             (cond
              ...
              ;; ---------------------------------------------------------------
              ;; virat
              ((anupamk:utils/host-name-is "kashi")
               '(t
                 :default-family "PragmataPro Mono"
                 :default-weight regular
                 :default-height 110))
...
(use-package zenburn-theme
  :unless (anupamk:utils/host-name-is "dirac")
  ...
  ;; ---------------------------------------------------------------------------
  ;; this looks nice !
  (setq default-frame-alist '((cursor-color . "#8faf9f")))
...
(use-package modus-themes
  :if (anupamk:utils/host-name-is "dirac")
...
(use-package emacs
  ...
  (define-key anupamk/global-keymap (kbd "C-c C-s C-n") #'anupamk:utils/create-new-scratch-buffer)
...
(use-package rg
  ...
  (rg-define-search anupamk:ripgrep-search/rg-vc-or-dir
    ...
    ;; current directory
    :dir (let ((project-root-dir (anupamk-locate-project-root-dirname)))
           (if project-root-dir
               project-root-dir
             default-directory))
...
(use-package protobuf-mode
  :if (anupamk:utils/host-name-is "aws-devel-02")
  :ensure t
  )
...
;; -----------------------------------------------------------------------------
;; ok, so `track-changes' package is one of the many specified by
;; `eglot' in it's 'Package-Requires: ...' section
;;
;; just include it here, and load it.
...
(use-package eglot-booster
  :ensure t
  :straight
  (:type git :host github :repo "jdtsmith/eglot-booster")
...
(use-package eglot
  :hook
  ;; ---------------------------------------------------------------------------
  ;; this is repetitive, but it's OK.
  (c-mode . eglot-ensure)
  (c++-mode . eglot-ensure))
...
(use-package cc-mode
  ...
  :init
  (defconst cc-lang-server-cmdline (anupamk:utils/get-attribute-value "cc-mode:lsp-server-with-args"))
  (defconst cc-clang-format-executable (anupamk:utils/get-attribute-value "cc-mode:clang-format-executable"))
...
Format =c-mode= or =c++-mode= buffers with =clang-format= when
available. This whole thing can be optimized a bit I think.
...
(use-package clang-format
  ...
  :config
  (setq clang-format-executable cc-clang-format-executable)
  )

(defun anupamk:cc/clang-fmtbuf-on-save ()
  "hook to apply 'clang-format-buffer' to a buffer before saving it"
...
(add-hook 'c-mode-hook (lambda () (anupamk:cc/clang-fmtbuf-on-save)))
(add-hook 'c++-mode-hook (lambda () (anupamk:cc/clang-fmtbuf-on-save)))
...
major-mode for python. Integrate this with eglot.
...
(use-package eglot
  ...
  :config
  ;; ---------------------------------------------------------------------------
  ;; first install 'gopls' for the language server
