...
#+begin_src emacs-lisp :tangle "early-init.el" :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "early-init.el"
  ...
  ;; -----------------------------------------------------------------------------
  ;; 1G !!! we are R.I.C.H
  (defvar anupam/gc-cons-threshold
    (* 1024 1024 1024))

  ;; -----------------------------------------------------------------------------
  ;; max amount that can be read from a process. this is '32mb' due to
  ;; lsp.
  (defvar anupam/process-max-memory
    (* 32 1024 1024))

  (defvar anupam/file-name-handler-alist file-name-handler-alist)

  (setq gc-cons-threshold most-positive-fixnum
        gc-cons-percentage 0.6
        file-name-handler-alist nil
        site-run-file nil
        read-process-output-max anupam/process-max-memory)

  ;; -----------------------------------------------------------------------------
  ;; run this function once init files are loaded.
  (defun anupam-early-init:after-loading-init-files-run ()
    ;; ---------------------------------------------------------------------------
    ;; restore it back
    (setq file-name-handler-alist anupam/file-name-handler-alist)

    ;; ---------------------------------------------------------------------------
    ;; garbage-collection can happen automatically once these many bytes
    ;; have been allocated since last garbage colletion.
    (setq gc-cons-threshold anupam/gc-cons-threshold)

    ;; ---------------------------------------------------------------------------
    ;; portion of heap used for allocation
    (setq gc-cons-percentage 0.1))

  (add-hook 'emacs-startup-hook  #'anupam-early-init:after-loading-init-files-run)

  ;; -----------------------------------------------------------------------------
  ;; disable garbage collection when in minibuffer
  (defun anupam-early-init:on-minibuffer-enter-run ()
    (setq gc-cons-threshold most-positive-fixnum))

  ;; -----------------------------------------------------------------------------
  ;; run when exiting the minibuffer
  (defun anupam-early-init:on-minibuffer-exit-run ()
    (garbage-collect)
    (setq gc-cons-threshold anupam/gc-cons-threshold))

  (add-hook 'minibuffer-setup-hook #'anupam-early-init:on-minibuffer-enter-run)
  (add-hook 'minibuffer-exit-hook #'anupam-early-init:on-minibuffer-exit-run)
...
#+begin_src emacs-lisp :tangle "init.el" :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "init.el"
  ...
  (require 'anupam-emacs-mode-line)
...
#+begin_src emacs-lisp :tangle "anupam-lisp/anupam-common-lib.el" :mkdirp yes  :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-lisp/anupam-prefix.el"  :tangle-mode (identity #o444)
  ...
  (defvar-keymap anupam-buffer-mode-map
    ...
    "t"      #'anupam-common:truncate-lines-silently
    "<end>"  #'end-of-buffer
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-host-customizations.el" :mkdirp yes :tangle-mode (identity #o444)
  ...
  (if (anupam-common:host-name-is "soma-debian-vm")
      (anupam:load-user-config-file "anupam-emacs-module/anupam-emacs-host-soma-debian-vm-customizations.el"))

#+end_src

** Host =shakti=

#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-host-shakti-customizations.el" :mkdirp yes :tangle-mode (identity #o444)
  ...
  (defconst anupam-font-family-name "PragmataPro Mono")

  ;; -----------------------------------------------------------------------------
  ;; '100m' file size is considered large here
  (defconst anupam-large-file-size-bytes (* 100 1024 1024))
  ...
  (defun anupam-cc-mode:lsp-server-ccls()
    (executable-find "ccls"))

  (defun anupam-cc-mode:lsp-server-ccls-args()
    (mapconcat #'identity `(
  ...
  ;; -----------------------------------------------------------------------------
  ;; '--compile-commands-dir' is needed for background indexing with
  ;; 'clangd' otherwise it just indexes files 'on-demand' i.e. as and
  ;; when they are opened.
  ;;
  ;; but '--compile-commands-dir' changes based on projects etc. so we
  ;; just use the '${PWD}' as the starting point.
  (defun anupam-cc-mode:lsp-server-clangd()
    "/usr/sbin/clangd")

  (defun anupam-cc-mode:lsp-server-clangd-args()
    (concat "--compile-commands-dir=" (getenv "PWD") " "
            (mapconcat #'identity `(
                            "-j=4"
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-host-soma-customizations.el" :mkdirp yes :tangle-mode (identity #o444)
  ...
  ;; host specific theme+font customizations
  (defconst anupam-theme-name "modus-operandi")
  (defconst anupam-font-family-name "Berkeley Mono")

  ;; ---------------------------------------------------------------------------
  ;; define font & size for modeline
  (defconst anupam-modeline-font "PragmataPro Mono")
  (defconst anupam-modeline-font-size 150)
  ...
  (defconst gcc-with-args (concat "/opt/homebrew/opt/gcc/bin/gcc-15 " gcc-plain-diag-args))
  (defconst g++-with-args (concat "/opt/homebrew/opt/gcc/bin/g++-15 " gcc-plain-diag-args))
  ...
  (defconst clang-with-args (concat "/opt/homebrew/opt/llvm/bin/clang" clang-plain-diag-args))
  (defconst clang++-with-args (concat "/opt/homebrew/opt/llvm/bin/clang++ " clang-plain-diag-args))
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-host-axon-customizations.el" :mkdirp yes :tangle-mode (identity #o444)
  ...
  ;; -----------------------------------------------------------------------------
  ;; '10m' file size is considered large here
  (defconst anupam-large-file-size-bytes (* 10 1024 1024))
  ...
  (defconst anupam-cc-mode:lsp-server-clangd-with-args '("/opt/homebrew/Cellar/llvm/20.1.6/bin/clangd"
                                                         "-j=2"
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-host-aws-devel-06-customizations.el" :mkdirp yes :tangle-mode (identity #o444)
  ...
  ;; ---------------------------------------------------------------------------
  ;; define font & size for modeline
  (defconst anupam-modeline-font "Berkeley Mono")
  (defconst anupam-modeline-font-size 150)
  ...
  ;; so for all code related actions, the picture looks something like this:
  ;;
  ;;    emacs == lsp-client (via a builtin package called lspce), and
  ;;    'external' language specific servers, for example:
  ;;
  ;;        - 'c' / 'c++' => 'clangd' / 'ccls'
  ;;        - 'golang'    => 'gopls'
  ...
  ;; -----------------------------------------------------------------------------
  ;; for ccls don't forget to do the following:
  ;;
  ;;     1. once 'compile_commands.json' is created, generate an
  ;;     offline-index for ccls to use: 'ccls-index=$PWD', *AND*
  ;;
  ;;     2. create an empty '.ccls-root' file in top-level i.e. the place
  ;;     where you just generated the offline-index at.
  ;;
  ;; the first one is the offline index ofcourse, and the 2nd one marks
  ;; the $ROOT of the project for which the index was created.
  ;;
  ;; heck, if you are creative enough, you can do #1 & #2 together !!!
  ;; what the world is coming to ? <snicker>
  ;; -----------------------------------------------------------------------------
  (defun anupam-cc-mode:lsp-server-ccls()
    "/twbuild/anupam.kapoor/local/usr/bin/ccls")


  (defun anupam-cc-mode:lsp-server-ccls-args()
    (mapconcat #'identity `(
                            "--log-file=/tmp/anupam-kapoor-ccls.log"
                            "-v=1"
                            )
               " "))

  (defun anupam-cc-mode:lsp-server-clangd()
    "/twbuild/anupam.kapoor/local/llvm/bin/clangd")
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-host-soma-debian-vm-customizations.el" :mkdirp yes :tangle-mode (identity #o444)

  ;;; anupam-emacs-host-soma-customizations.el -- soma-debian-vm-host specific customizations -*- lexical-binding:t -*-
  ...
  ;; -----------------------------------------------------------------------------
  ;; '32m' file size is considered large here
  (defconst anupam-large-file-size-bytes (* 32 1024 1024))
  ...
  ;; what the world is coming to ? <snicker>
  ;; -----------------------------------------------------------------------------
  (defconst anupam-cc-mode:lsp-server-ccls-with-args '("/usr/bin/ccls"
                                                       "--log-file=/tmp/ccls.log"
                                                       "--log-file-append=false"
                                                       "-v=1"
                                                       ))

  (defconst anupam-cc-mode:lsp-server-clangd-with-args '("/usr/bin/clangd-19"
                                                         "--enable-config"
                                                         "-j=8"
                                                         "--malloc-trim"
                                                         "--all-scopes-completion=1"
  ...
  ;; -----------------------------------------------------------------------------
  ;; setup less noisy flags.
  ;; see the gcc man-page for more information.
  (defconst gcc-plain-diag-args
    (concat "-fno-diagnostics-show-caret "
            "-fdiagnostics-color=never "
            "-fdiagnostics-path-format=separate-events "
  ...
  (defconst gcc-with-args (concat "/usr/bin/gcc-14 " gcc-plain-diag-args))
  (defconst g++-with-args (concat "/usr/bin/g++-14 " gcc-plain-diag-args))
  ...
  (defconst clang-with-args (concat "/usr/bin/clang-19 " clang-plain-diag-args))
  (defconst clang++-with-args (concat "/usr/bin/clang++-19 " clang-plain-diag-args))
  ...
  ;; -----------------------------------------------------------------------------
  ;; where is `clang-format-executable' ?
  (defconst anupam-cc-mode:clang-format-executable "/usr/bin/clang-format-19")
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-theme.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-mode-line.el" :tangle-mode (identity #o444)
  ;;; anupam-emacs-mode-line.el --- make the modeline pretty -*- lexical-binding: t; -*-

  ;; ==============================================================================
  ;; define the faces that we might use here
  (defface anupam-modeline--indicator-red-bg
    '(
    ...
    "modeline indicator with a red background.")

  (defface anupam-modeline--indicator-blue-bg
    '(
      (((class color) (min-colors 88) (background light))
    ...
    "modeline indicator with a blue background.")

  ;; -----------------------------------------------------------------------------
  ;; STEP-1 : Have the vanilla thing first
  ;; =====================================
  ;;
  ;; NOTES
  ;;    - ':eval' here makes the evaluation of the form that follows,
  ;;    dynamic. thus, 'mode-line-format' is computed anew in every
  ;;    buffer, instead of having a fixed recorded value at the place
  ;;    where 'setq-default' was invoked.
  ;;
  ;;    everything else is statically defined.
  ;;
  ;;    - 'propertize' adds various properties to the string that is
  ;;    returned for the purposes of displaying on the modeline.
  ;;
  ;;    builtin properties can be obtained by the 'list-faces-display',
  ;;    and then used.
  ;;
  ;;    these properties are defined via the 'defface' macro, and you
  ;;    can define your own as well.
  ;;
  ;;    And we get this:
  ;;    ,----
  ;;    | (setq-default
  ...
  ;;    problems are:
  ;;        - looks ugly,
  ;;        - all in 1 single line.
  ;;    we don't have to live like this, and we can do better !
  ;;
  ;;    specifically, we can take each of the 'buffer-name' /
  ;;    'major-mode' etc. and put them in variables.
  ;;
  ;; STEP-2 : Put the modeline constructions in variables
  ;; ====================================================
  ;;
  ;; 1. this is done with the 'defvar-local' declaration for declaring
  ;; local variables.
  ;;
  ;; 2. all the variables must have the 'risky-local-variable' property
  ;; associated with them.
  ;; -----------------------------------------------------------------------------
  (setq-default mode-line-format '(""
                                   anupam-modeline--remote-buffer-indicator
                                   anupam-modeline--buffer-modified-indicator
                                   anupam-modeline--buffer-name-info
                                   mode-line-format-right-align
                                   anupam-modeline--git-info
                                   "  "
                                   anupam-modeline--major-mode
                                   " "
                                   anupam-modeline--date-time-info
                                   " "
                                   ))

  ;; -----------------------------------------------------------------------------
  ;; make all variables risky
  (dolist (modeline-var-i '(anupam-modeline--remote-buffer-indicator
                            anupam-modeline--buffer-modified-indicator
                            anupam-modeline--buffer-name-info
                            anupam-modeline--major-mode
                            anupam-modeline--git-info
    ...
    (put modeline-var-i 'risky-local-variable t))

  ;; ==============================================================================
  ;; buffer-remote ?
  (defvar-local anupam-modeline--remote-buffer-indicator
    '(:eval
      (when (file-remote-p default-directory)
        (propertize " @ "
                    'face 'anupam-modeline--indicator-red-bg)))
    "mode line definition for remote buffers.")

  ;; ==============================================================================
  ;; buffer-modified ?
  (defvar-local anupam-modeline--buffer-modified-indicator
    '(:eval
      (when (buffer-modified-p)
        (propertize " * "
                    'face 'anupam-modeline--indicator-blue-bg)))
    "mode line definition for modified buffers.")

  ;; ==============================================================================
  ;; buffer-name-info
  (defun anupam-modeline--buffer-name-with-ro-ind ()
    "Return buffer name with r(ead) o(nly) indicator('') if necessary."
    (if buffer-read-only
        (format " %s %s" (char-to-string #xF023) (buffer-name))
      (buffer-name)))

  (defun anupam-modeline--buffer-location ()
    "Return location of point in the buffer."
    (format-mode-line "[%l:%c]"))

  (defvar-local anupam-modeline--buffer-name-info
    '(:eval
      (concat (anupam-modeline--buffer-name-with-ro-ind)
              " "
              (anupam-modeline--buffer-location)))
    "mode line definition for displaying buffer-name")

  ;; ==============================================================================
  ;; major-mode
  (defun anupam-modeline--major-mode-indicator ()
    "Return indicator for major-mode"

    (let ((mm-ind-value (cond ((derived-mode-p 'text-mode) "§")
                              ((derived-mode-p 'prog-mode) "𝞴")
                              (t "◦"))))
      (propertize mm-ind-value 'face 'shadow)))

  (defun anupam-modeline--major-mode-short-name ()
    "Short capitalized major mode name"
    (capitalize (string-replace "-mode" "" (symbol-name major-mode))))

  (defvar-local anupam-modeline--major-mode
    '(:eval
      (concat (anupam-modeline--major-mode-indicator)
              " "
              (propertize (anupam-modeline--major-mode-short-name) 'face 'shadow)))

    "mode line definition for displaying major-mode")

  ;; ==============================================================================
  ;; git-version-control information for the visited buffer when
  ;; appropriate.

  ;; -----------------------------------------------------------------------------
  ;; action to execute when 'mouse-1' is clicked on highlighted branch-indicator.
  (defvar anupam-modeline--git-info-mouse-map
    (let ((git-info-mouse-map (make-sparse-keymap)))
      (define-key git-info-mouse-map
                  [mode-line down-mouse-1] 'vc-diff)
      git-info-mouse-map)
    "Configure mouse-actions when clicked on branch-indicator on the modeline.")

  ;; ---------------------------------------------------------------------------
  ;; help-text to display when mouse hovers on highlighted branch-indicator.
  (defun anupam-modeline--git-info-text-show-help (file-name)
    "Return help message for FILE-NAME tracked by git."
    (format "Revision: %s\nmouse-1: `vc-diff'"
            (vc-git-working-revision file-name)))

  ;; -----------------------------------------------------------------------------
  ;; git information that will be shown on the modeline. 'diffstat' is
  ;; just a bit too much for my taste.
  (defun anupam-modeline--git-info-text (file-name branch-name)
    "Text for git controlled file and branch names"
    (concat
     "["
     ...
     (propertize branch-name
                 'face 'shadow
                 'mouse-face 'ansi-color-inverse
                 'help-echo (anupam-modeline--git-info-text-show-help file-name)
                 'local-map anupam-modeline--git-info-mouse-map)
     "]"
     ))

  (defvar-local anupam-modeline--git-info
    '(:eval
      (when-let* ((file-name (buffer-file-name))
                  ;; -----------------------------------------------------------
                  ;; file is registered with git ?
                  (vc-git-registered file-name)

                  ;; -----------------------------------------------------------
                  ;; get git-branch-name
                  (branch-name (vc-git--symbolic-ref file-name)))
        (anupam-modeline--git-info-text file-name branch-name)))
    "mode line definition for displaying version control information")

  ;; ==============================================================================
  ;; date-time-info
  ;;
  ;; i use the venerable 'format-time-string' here. syntax reminder:
  ;;     '%d' => (zero-padded) day of the month
  ;;     '%b' => abbreviated name of the month
  ;;     '%H' => hour on 24h clock
  ;;     '%M' => minute (00..59)
  (defvar-local anupam-modeline--date-time-info
    '(:eval
      (propertize (format-time-string " ❬ %d-%B ❙ %H:%M ❭ ") 'face 'shadow))
    "mode line definition for displaying date and time")

  ;; -----------------------------------------------------------------------------
  ;; setup modeline fonts to be different than the buffer fonts.
  (set-face-attribute 'mode-line nil :font anupam-modeline-font :height anupam-modeline-font-size)
  (set-face-attribute 'mode-line-inactive nil :font anupam-modeline-font :height anupam-modeline-font-size)

  ;; -----------------------------------------------------------------------------
  ;; last line of this file
  (provide 'anupam-emacs-mode-line)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-modus-operandi-theme.el" :tangle-mode (identity #o444)

  ;;; anupam-emacs-modus-operandi-theme.el --- modus operandi tweaks -*- lexical-binding: t; -*-
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-modus-vivendi-theme.el" :tangle-mode (identity #o444)

  ;;; anupam-emacs-modus-vivendi-theme.el --- modus vivendi tweaks -*- lexical-binding: t; -*-
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-zenburn-theme.el" :tangle-mode (identity #o444)

  ;;; anupam-emacs-zenburn-theme.el --- zenburn tweaks -*- lexical-binding: t; -*-
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-theme.el"
  ...
  ;;    - 'zenburn'
  ;;
  ;; these themes are very slightly tweaked to my tastes
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-theme.el"
  ...
  (use-package fontaine
    ...
    ;; ---------------------------------------------------------------------------
    ;; defer loading 'fontaine' it seems to be slowing things down
    ;; significantly.
    :defer t
    ...
    ;; ---------------------------------------------------------------------------
    ;; quasi-quoting in lisp is quite useful, as demonstrated here.
    ;;
    ;; fonts are not specified with a name, but through the
    ;; 'anupam-font-family-name' variable. this variable specifies the
    ;; font-family to use.
    ;;
    ;; for now just a single family is sufficient for 'fixed-pitch',
    ;; 'variable-pitch' as well as 'default-family'. when needed, these
    ;; can always be broken out into something more fancier.
    (setq fontaine-presets `((small
                              :default-family ,anupam-font-family-name
                              :fixed-pitch    ,anupam-font-family-name
                              :variable-pitch ,anupam-font-family-name
  ...
  ;; `fontaine-presets' (the `regular' in this case).
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"
  ...
  (use-package emacs
    ...
    ;; ---------------------------------------------------------------------------
    ;; more information in completions
    ...
    ;; ---------------------------------------------------------------------------
    ;; enforce utf-8 as the default coding system for all files, buffers
    ;; and comint processes.
    ...
    ;; ---------------------------------------------------------------------------
    ;; a file is considered 'large' based on the 'host-specific' value
    ;; of 'anupam-large-file-size-bytes'
    (setq large-file-warning-threshold anupam-large-file-size-bytes)
    ...
    ;; ---------------------------------------------------------------------------
    ;; question: how do you undo an inadvertent motion ? something like
    ;; 'winner-mode' but for the point.
    ;;
    ;; you can invoke 'C-u C-<space>' to jump back to where 'mark' was
    ;; (you don't need to set it beforehand)
    ;;
    ;; with 'set-mark-command-repeat-pop' you can do
    ;; 'C-u C-<space> C-<space> C-<space> ...' for great good.
    ...
    ;; ---------------------------------------------------------------------------
    ;; echo unfinished commands after these many seconds of pause
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"
  ...
  ;; -----------------------------------------------------------------------------
  (bind-keys* :prefix "C-c ," :prefix-map ctrl-c-comma-map)
  (bind-keys* :prefix "C-c c" :prefix-map ctrl-c-c-map)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"
  ...
  (use-package emacs-autorevert
    ...
    ;; ---------------------------------------------------------------------------
    ;; revert files without any query
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"
  ...
  (use-package crux
    ...
    :bind (("C-a"           . crux-move-beginning-of-line)
           ...
           :map ctrl-c-c-map
           ("<return>" . crux-switch-to-previous-buffer))
...
Increase the selected region by semantic units. This is slightly more
intuitive than =hyperbole=

#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"

  (use-package expand-region
    :ensure t
    :bind (:map ctrl-c-c-map
                ("e" . er/expand-region))
    )
...
#+end_src
*** Unbold fonts ... puhleease !
:PROPERTIES:
:CUSTOM_ID: h:7b7dd48e-17a6-4002-9a35-50945b4c721a
:END:
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-unannoy.el"
  ...
  (add-hook 'after-load-theme-hook #'anupam-common:unannoy-fonts)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-window.el" :mkdirp yes :tangle-mode (identity #o444)
...
When the =repeat-mode= is enabled, via the =M-x repeat-mode=,
repeatedly pressing the =<= or =>= keys cycles through the window
configuration.
...
:PROPERTIES:
:CUSTOM_ID: h:62B72363-B491-4124-825E-F28A5143E408
:END:

Monitors are getting wider than they are getting taller. Emacs's
defaults in 'split-windows-sensibly' is at odds with this trend. Let's
just fix that here.
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-window.el"
  ...
  ;; -----------------------------------------------------------------------------
  ;; fix annoying vertical window splitting, from:
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-window.el"
  ...
  (use-package popper
    ...
    ;; ---------------------------------------------------------------------------
    ;; group by project.el 'project-root', falling back to
    ;; default-directory
    (setq popper-group-function #'popper-group-by-directory)

    ;; ---------------------------------------------------------------------------
    ;; popup placement is controlled by 'display-buffer-alist'
    ;; configuration.
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-utilities.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-completion.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-completion.el"
  ...
  ;; -----------------------------------------------------------------------------
  ;; define a face for the prefix-character used to display current
  ;; candidates in vertico completion list.
  (defface anupam-vertico-prefix-face
  ...
  (use-package vertico
    ...
    :custom
    ;; ---------------------------------------------------------------------------
    ;; only '10' items if you please.
    (vertico-count 10)

    ;; ---------------------------------------------------------------------------
    ;; disable resize of vertico buffer
    (vertico resize nil)

    ;; ---------------------------------------------------------------------------
    ;; don't cycle through the candidate when reaching the end of the
    ;; list
    (vertico-cycle nil)

    :config
    ;; ---------------------------------------------------------------------------
    ;; customize display of current candidate in the completion list by
    ;; prefixing him with “» ” to make it stand out.
    ;;
    ;; from:
                ...
                (lambda (orig cand prefix suffix index _start)
                  ...
                  (concat
                   (if (= vertico--index index)
                       (propertize "‣ " 'face 'anupam-vertico-prefix-face)
                     "  ")
                   cand)))
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-search.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-search.el"
  ...
  (use-package rg
    ...
    :config
    ;; ---------------------------------------------------------------------------
    ;; project-wide and current-working-directory custom searches.
    (rg-define-search anupam-rg:project-root-or-cwd
      :query ask
      :format regexp
      :files "everything"
      :dir (let ((proj-root-dir (vc-root-dir)))
             (if proj-root-dir
                 proj-root-dir
               default-directory))
      :confirm prefix
      :flags ("--hidden -g '!third_party/'"))

    (rg-define-search anupam-rg:cwd
      :query ask
      :format regexp
      :files "everything"
    ...
    :bind (("M-s p" . anupam-rg:project-root-or-cwd)
           ("M-s d" . anupam-rg:cwd)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-dired.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-version-control.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el" :mkdirp yes :tangle-mode (identity #o444)
...
[[https://github.com/zbelial/lspce][LSPCE]] is a simple LSP client that is implemented as an Emacs module.

Lets set that up here:
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el"
  ...
  (use-package lspce
    :ensure nil
    :load-path (lambda() (concat user-emacs-directory "lspce"))
    :config
    ...
    (setq lspce-enable-eldoc nil)
    (setq lspce-eldoc-enable-hover nil)
    (setq lspce-eldoc-enable-signature nil)
    (setq lspce-enable-flymake nil)

    ;; ---------------------------------------------------------------------------
    ;; where lspce does its logging ?
    (lspce-set-log-file "/tmp/lspce.log")
    (lspce-enable-logging)

    ;; ---------------------------------------------------------------------------
    ;; update 'lspce-server-programs' to use appropriate lsp server for
    ;; different programming modes.
    (setq lspce-server-programs `(("C" anupam-cc-mode:lsp-server-clangd  anupam-cc-mode:lsp-server-clangd-args))
          ))
...
:PROPERTIES:
:CUSTOM_ID: h:8046d388-d424-4af8-9790-005e4fad2c83
:END:

[[https://www.flycheck.org/en/latest/index.html][Flycheck]] is a =on the fly= linter frontend for Emacs. I just enable it
here. Programming language specific configuration f.e. which version
to check for etc. is done in their respective sections.

I tried using the builtin flymake earlier, but it was just too slow
for my tastes. Lets see how this goes.
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el"
  ...
  ;; -----------------------------------------------------------------------------
  ;; disable flycheck (on typical emacs configs, produces far more false
  ;; positives than useful warnings).
...
**** Meson Integration
:PROPERTIES:
:CUSTOM_ID: h:281b31eb-41db-43ad-b82d-b876f0ac2536
:END:

#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el"

  ;; -----------------------------------------------------------------------------
  ;; no configuration is necessary i hope
...
#+begin_src emacs-lisp :tangle "anupam-lisp/anupam-cc-mode-lib.el" :mkdirp yes :tangle-mode (identity #o444)
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el"
  ...
  ;; -----------------------------------------------------------------------------
  ;; flycheck use c99 for C
  (defun anupam-cc-mode:flycheck-c-standard ()
    (setq flycheck-gcc-language-standard "c99")
    (setq flycheck-clang-language-standard "c99"))

  ;; -----------------------------------------------------------------------------
  ;; flycheck use c++20 for C++
  (defun anupam-c++-mode:flycheck-c++-standard ()
  ...
  (use-package cc-mode
    ...
    :bind(
          ...
          ("C-<return>"    . #'recompile)
          ("C-c <return>"  . #'anupam-cc-mode:quick-compile))

    :hook
    (c-mode   . anupam-cc-mode-hook)
    (c++-mode . anupam-cc-mode-hook)
    (c++-mode . anupam-c++-mode:flycheck-c++-standard)
    (c-mode   . anupam-cc-mode:flycheck-c-standard)
...
Lots of compilers f.e. =gcc= =clang= now support color output. Instead
of disabling that like a caveman, let's just use it.
...
****** LSPCE configuration
:PROPERTIES:
:CUSTOM_ID: h:ef228d53-cd3a-4cb6-aa62-8f844a841443
:END:
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-langs.el"
  ...
  (use-package python-mode
    ...
    :hook
    ((python-mode . company-mode-quicker)))

  (use-package lazy-ruff
    :ensure t

    :bind
    (("C-c f" . lazy-ruff-lint-format-dwim))
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-org-mode.el" :mkdirp yes :tangle-mode (identity #o444)
...
Just some assorted tips and tricks gathered from the ether.

** Narrow/Widen to/from region

The command =narrow-to-region= lets you narrow the current buffer
i.e. hide all text other than the selected text. Useful from
time-to-time to focus on current portion of the buffer.

It can be undone with =widen=.
...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-config-end.el" :tangle-mode (identity #o444)
