...
#+begin_src emacs-lisp :tangle "anupam-emacs-module/anupam-emacs-mode-line.el"
  ...
  (dolist (modeline-var-i '(anupam-modeline--buffer-name
                            anupam-modeline--major-mode
                            anupam-modeline--git-info
                            anupam-modeline--date-time-info
                            ))
  ...
  ;; ==============================================================================
  ;; buffer-name
  (defvar-local anupam-modeline--buffer-name
      '(:eval
        ;; -----------------------------------------------------------------------
        ;; 'format-mode-line' follows the same format specification as
        ;; the 'mode-line-format', let's just use that:
        ;;
        ;; syntax reminder:
        ;;    '%@' => print '@' for remote files, '-' otherwise
        ;;    '%&' => print '*' if buffer is modified, '-' otherwise
        ;;    '%b' => print 'buffer-name'
        ;;    '%l' => print 'line-number'
        ;;    '%c' => print 'column-number'
        ;;
        ;; now, most of the time, both '%@' and '%&' will display
        ;; '-'. so there is no need to waste / clutter the precious
        ;; modeline space with this.
        ;;
        ;; we can do that(this is Emacs afterall). but more expertise is
        ;; needed.
        ;; -----------------------------------------------------------------------
        (propertize (format-mode-line "[%@%&] %b [%l:%c]") 'face 'bold))
